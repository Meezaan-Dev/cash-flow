...
const SettingsModal: React.FC<SettingsModalProps> = ({
	...
	useEffect(() => {
		if (!mainAccountId || selectedMainAccountExists || accounts.length === 0) return;
		setMainAccountId(accounts[0].id ?? '');
	}, [accounts, mainAccountId, selectedMainAccountExists, setMainAccountId]);
									...
									<div>
										...
										<div className="space-y-4 rounded-lg border p-4 sm:p-5">
											...
											<div className="space-y-2">
												<Label htmlFor="main-account">
													Main account for new transactions
												</Label>
												<select
													id="main-account"
													value={
														selectedMainAccountExists
															? mainAccountId
															: accounts[0]?.id ?? ''
													}
													onChange={(event) =>
